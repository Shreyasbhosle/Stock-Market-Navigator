...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:@localhost:3308/stock'  
...
def save_four_predictions_plot(actual_data, future_predictions, filename):
    ...
    plt.plot(days, future_predictions, label='Predicted Stock Prices', color='red')
    plt.xticks(np.arange(1, len(future_predictions) + 1, 1))
...
def save_half_predictions_plot(actual_data, future_predictions, filename):
    ...
    days = np.arange(1, len(future_predictions) + 1)
    plt.plot(days, future_predictions, label='Predicted Stock Prices', color='red')
    plt.xticks(np.arange(1, len(future_predictions) + 1, 15))
...
def save_year_predictions_plot(actual_data, future_predictions, filename):
    ...
    days = np.arange(1, len(future_predictions) + 1)
    plt.plot(days, future_predictions, label='Predicted Stock Prices', color='red')
    plt.xticks(np.arange(1, len(future_predictions) + 1, 30))
...
def calculate_debt_to_equity_ratio(debt, equity):
    return debt / equity if equity != 0 else None

def calculate_pe_ratio(stock_price, earnings_per_share):
    return stock_price / earnings_per_share if earnings_per_share != 0 else None

def calculate_earning_yield(earnings_per_share, stock_price):
    return earnings_per_share / stock_price if stock_price != 0 else None

def calculate_pe_to_growth_ratio(pe_ratio, growth_rate):
    return pe_ratio / growth_rate if growth_rate != 0 else None
...
def get_financial_data(symbol):
    ...
    total_debt = balance_sheet['Total Debt'].iloc[-1]  # Get latest total debt
    total_equity = balance_sheet['Total Equity'].iloc[-1]  # Get latest total equity
    
    # Get income statement data
    income_statement = stock_data.financials
    net_income = income_statement['Net Income'].iloc[-1]  # Get latest net income
    
    # Calculate EPS
    outstanding_shares = stock_data.info['sharesOutstanding']
    earnings_per_share = net_income / outstanding_shares
    
    # Get current stock price
    current_stock_price = stock_data.history(period='1d')['Close'].iloc[-1]  # Get latest closing price
    
    return total_debt, total_equity, earnings_per_share, current_stock_price
...
def invest():
    if session.get('user_id'):
        if request.method == 'POST':
            ...
            investment_amount = int(amount)  # Investment amount
            ...
            # Ratio
            #total_debt, total_equity, earnings_per_share, current_stock_price = get_financial_data(symbol)
            #print(total_debt, total_equity, earnings_per_share, current_stock_price)
